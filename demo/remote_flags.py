...
async def async_demo():
    ...
    async with mixpanel.Mixpanel(
        PROJECT_TOKEN, remote_flags_config=remote_config
    ) as mp:
        variant_value = await mp.remote_flags.aget_variant_value(
            FLAG_KEY, FLAG_FALLBACK_VARIANT, USER_CONTEXT
        )
...
def sync_demo():
    ...
    with mixpanel.Mixpanel(PROJECT_TOKEN, remote_flags_config=remote_config) as mp:
        variant_value = mp.remote_flags.get_variant_value(
            FLAG_KEY, FLAG_FALLBACK_VARIANT, USER_CONTEXT
        )
        print(f"Variant value: {variant_value}")
