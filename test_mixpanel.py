...
class TestMixpanelPeople(TestMixpanelBase):
    def test_people_set(self):
        self.mp.people_set(
            "amq", {"birth month": "october", "favorite color": "purple"}
        )
        assert self.consumer.log == [
            (
                "people",
    ...
    def test_people_meta(self):
        self.mp.people_set(
            "amq",
            {"birth month": "october", "favorite color": "purple"},
...
class TestMixpanelGroups(TestMixpanelBase):
    def test_group_set(self):
        self.mp.group_set(
            "company", "amq", {"birth month": "october", "favorite color": "purple"}
        )
        assert self.consumer.log == [
            (
                "groups",
    ...
    def test_custom_json_serializer(self):
        ...
        with pytest.raises(TypeError) as excinfo:
            self.mp.track(
                "ID", "button press", {"size": decimal.Decimal(decimal_string)}
            )
        ...
        self.mp.track(
            "ID",
            "button press",
            {"size": decimal.Decimal(decimal_string), "$insert_id": "abc123"},
        )
        assert self.consumer.log == [
            (
                "events",
    ...
    def test_server_invalid_data(self):
        with responses.RequestsMock() as rsps:
            ...
            rsps.add(
                ...
                match=[
                    urlencoded_params_matcher(
                        {"ip": "0", "verbose": "1", "data": '{INVALID "foo":"bar"}'}
                    )
                ],
            )
    ...
    def test_track_functional(self):
        with responses.RequestsMock() as rsps:
            ...
            self.mp.track(
                "player1",
                "button_press",
                {"size": "big", "color": "blue", "$insert_id": "xyz1200"},
            )
...
class TestServiceAccountAuth:
    ...
    @responses.activate
    def test_api_secret_credentials_basic_usage(self):
        """Test APISecretCredentials can be used for authentication."""
        responses.add(
            responses.POST,
            "https://api.mixpanel.com/track",
            json={"status": 1, "error": None},
        ...
        credentials = mixpanel.APISecretCredentials(api_secret=self.API_SECRET)
        consumer = mixpanel.Consumer(credentials=credentials)
        consumer.send("events", '{"event": "test"}', api_key="test-key")

        # Verify the request was made with Basic Auth (secret:empty password)
        assert len(responses.calls) == 1
        request = responses.calls[0].request
        auth_header = request.headers.get("Authorization")
        assert auth_header is not None
        # Verify it's using the API secret as username with empty password
        assert auth_header == self._verify_basic_auth(self.API_SECRET, "")

    def test_api_secret_credentials_validation(self):
        """Test APISecretCredentials requires non-empty secret."""
        with pytest.raises(ValueError, match="API secret cannot be empty"):
            mixpanel.APISecretCredentials(api_secret="")

    def test_api_secret_credentials_repr_hides_secret(self):
        """Test APISecretCredentials __repr__ doesn't expose the secret."""
        credentials = mixpanel.APISecretCredentials(api_secret="my-secret")
        repr_str = repr(credentials)
    ...
    @responses.activate
    def test_api_secret_credentials_with_mixpanel(self):
        """Test APISecretCredentials works with Mixpanel class."""
        responses.add(
            responses.POST,
            "https://api.mixpanel.com/track",
            json={"status": 1, "error": None},
